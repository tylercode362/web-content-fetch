...
function renderHtml(token) {
  return '<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8">' +
    ...
    'const decorateJobs=jobs=>{[...document.querySelector("#jobs").children].forEach((li,index)=>{const job=jobs[index];if(!job)return;const identity=document.createElement("div");identity.className="identity";identity.textContent="binding "+(job.bindingId||"-")+"｜browser "+(job.browserClientId||"-")+"｜service "+(job.serviceClientId||"-")+"｜"+(job.bridgeUrl||"-");li.append(identity);const addAction=(action,label)=>{const button=document.createElement("button");button.dataset.jobId=job.id;button.dataset.action=action;button.textContent=label;li.append(document.createTextNode(" "));li.append(button);};if(["queued","running"].includes(job.status))addAction("pause","暫停");if(job.status==="paused")addAction("resume","恢復");if(["queued","running","pausing","paused","cancelling"].includes(job.status))addAction("cancel","取消並刪除");if(["complete","error","cancelled"].includes(job.status))addAction("delete","刪除記錄與輸出");});};' +
    'const render=data=>{renderBinding(data.binding||{});document.querySelector("#jobs").innerHTML=(data.jobs||[]).map(j=>"<li><strong>"+esc(j.status)+"</strong> "+esc(j.kind)+" "+esc(j.title||j.url)+" <small>"+esc(JSON.stringify(j.progress||{}))+(j.diagnostic?" "+esc(j.diagnostic):"")+"</small>"+(j.outputs||[]).map(o=>" <a href=\\""+esc(o)+"\\">下載</a>").join("")+"</li>").join("");};' +
    'async function refresh(){const r=await fetch("/api/state");if(!r.ok)throw new Error("state_http_"+r.status);const data=await r.json();const binding=data.binding||{};document.querySelector("#bridgeUrl").value=binding.bridgeUrl||"";document.querySelector("#callbackUrl").value=binding.callbackUrl||"";render({...data,binding});decorateJobs(data.jobs||[]);}' +
    ...
    'const events=new EventSource("/api/events");events.onmessage=e=>{const data=JSON.parse(e.data);render(data);decorateJobs(data.jobs||[]);};refresh();' +
    '</script></body></html>';
}
